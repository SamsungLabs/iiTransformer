# Option utility code adapted from KAIR (https://github.com/cszn/KAIR)
...
def parse(opt_path, is_train=True):
    ...
    for phase, dataset in opt['datasets'].items():
        ...
        if 'dataroot_H' in dataset and dataset['dataroot_H'] is not None:
            if not isinstance(dataset['dataroot_H'], list):
                dataset['dataroot_H'] = os.path.expanduser(dataset['dataroot_H'])
            else:
                for i, d_root in enumerate(dataset['dataroot_H']):
                    dataset['dataroot_H'][i] = os.path.expanduser(d_root)
